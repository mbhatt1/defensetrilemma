...
attacker, it faces a structural asymmetry:

\begin{theorem}[Capacity Parity Disadvantage]
\label{thm:capacity}
The defense must satisfy two constraints simultaneously---correct
classification (distinguishing $S_\tau$ from $U_\tau$) and utility
preservation (identity on $S_\tau$)---while the attacker need only
find one input in $U_\tau$. The defense's hypothesis class must
represent both the decision boundary and the identity function on
the safe side, while the attacker's search is unconstrained.
\end{theorem}

Utility preservation is a structural tax on the defense that the
attacker does not pay: the attacker explores freely, while the
defense must simultaneously preserve and filter.
...
\paragraph{5. Break the asymmetry with architectural separation.}
The defense must simultaneously classify and preserve
(\Cref{thm:capacity}), while the attacker only searches. Separate
the defense from the utility pipeline---e.g., a lightweight
classifier that flags, paired with a separate response
generator---to decouple the two tasks.
