...
% TODO: Agent 4 is wiring a --judge flag in trilemma_validator/cli.py
% but will likely not produce live data (no API key available at
% edit time). Replace the placeholder below with the two-judge
% comparison once trilemma_validator/live_runs/judge_robustness/
% lands.
...
thereby change $\hat L, \hat\ell, \hat G$. Full two-judge
re-evaluation pending; see
\texttt{trilemma\_validator/live\_runs/judge\_robustness\_pending.md}.
A comprehensive judge-robustness study (multiple judges,
calibration, agreement diagnostics) is left to future work.
