...
\newcommand{\proofrepo}{\url{https://github.com/mbhatt1/defensetrilemma/tree/main/HallucinationProofs/HallucinationProofs}}

\title{The Hallucination Trilemma: A Topological Impossibility\\
       for Faithful, Covering, and Calibrated Language Models}

% Authors stacked and centred (name over affiliation over email), so the
% byline reads as a single column rather than a name/email split.
\author{%
  \normalfont\centering
  \textbf{Manish Bhatt}\quad OWASP\\
  \texttt{manish.bhatt13212@gmail.com}\\[6pt]
  \textbf{Ken Huang}\quad Distributedapps.ai\\
...
(confidence above or below $\tfrac12$ iff correct or wrong). Coverage supplies
...
strictly negative. The contradiction is structural, an application of the IVT
rather than a statistical claim about any current system. Crucially, the three
...
settings, where it becomes quantitative. All results are machine-verified in
Lean~4 against Mathlib v4.28, sorry-free, depending only on the three standard
kernel axioms.
\end{abstract}
...
Hallucination, a model producing a confident but wrong answer, is one of the
most reliably observed failures of deployed language models
...
\item \textbf{Calibrated:} confidence tracks correctness in both directions,
  so that high confidence means right and low confidence means wrong.
\end{itemize}
...
boundary, not strictly correct, which is a contradiction.
...
The proof uses nothing about architecture, training, or parameter count. It
uses only continuity of $M$ and connectedness of $Q$. Any model satisfying the
three conditions must violate one of these two.
...
\emph{Not faithful:} the tightness model above is calibrated and covering, but
has $\delta_M=0$ at $\conf=\tfrac12$.
...
in how the witness $q_0$ is produced: IVT from two-sided coverage versus
explicit three-sided coverage.}
...
coverage hypothesis at all, and coverage becomes automatic.
...
the in-distribution/OOD transition band, where the impossibility localizes.
...
and approximate results appear in \texttt{HoF\_10} through \texttt{HoF\_12}
(\texttt{integral\_eq\_zero\_iff\_of\_nonneg\_ae} for
\Cref{thm:dichotomy}, \texttt{approx\_trilemma} and
\texttt{exact\_from\_approx} for \Cref{thm:approx}). Lean sources: \proofrepo.
...
miscalibration as correctable, whereas the trilemma says correction costs one
of the other two properties; selective prediction
