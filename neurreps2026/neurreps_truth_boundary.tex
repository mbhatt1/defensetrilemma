...
kernel axioms. Experiments observe the predicted coupling in five
...
and Qwen2.5-1.5B, on the 1496 matched city statements and
negations of \citet{marks2024geometry}.

\paragraph{Setup.}
We embed each statement at its final token, select the layer whose
probe validates best, train the truth field $\dF$ as a linear
logistic probe on one city split and the confidence field $\conf$
as a second head on a disjoint split with a different seed, so the
two fields are independently trained readouts of the same space,
the canonical construction the theory models. We interpolate hidden
states from each true statement to its negation, record both fields,
and keep paths whose endpoints the probe classifies correctly, 83,
104, 137, 66, and 148 paths respectively, with test cities held out
from both splits.

\begin{table}[t]
\centering
\caption{Boundary coupling in five small models. Probe and Head
are held-out accuracies, Verbal the model's own answer accuracy,
Cross the mean confidence at the probe's zero crossing, Gap the
median crossing distance as a fraction of the path, Width the
relative boundary width.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\linewidth]{boundary_coupling.pdf}\\[2pt]
\includegraphics[width=0.74\linewidth]{endogenous_coupling.pdf}
...
crossing where confidence passes one half in every model. Lower pair,
the same with the model's own probability of answering true from
patched final-block states, decisive paths only. The Qwens sweep
through one half at the boundary, Pythia-410M answers true
everywhere so the premise fails, and GPT-2 and Phi-1.5 sit between.}
\label{fig:coupling}
\end{figure}
...
the two crossings tracks probe accuracy monotonically, 0.31 for
...
with no layer of its 24 probing above 0.73, while the Qwen pair
...
paths. A path satisfies the separation premise when this field is
decisive, on the correct side of one half, at both endpoints. \tableref{tab:endogenous} and \figureref{fig:coupling} report
...
\begin{table}[t]
\centering
\caption{Endogenous confidence and null calibration. Decisive
counts endpoint-decisive paths, Endo the median gap on them, Perm
and Rand the median null gaps over twenty seeds, Heads the
trained-head gap.}
...
Permuted-label and random-direction heads, twenty seeds each on the
same kept paths, calibrate the gaps (\tableref{tab:endogenous}).
The trained-head gaps of 0.25, 0.20, 0.10, 0.31, and 0.05 fall
below every null median, and below the entire null range for
Pythia-410M and both Qwens, while for GPT-2 and Phi-1.5 they lie
inside the random-head range, weak evidence on its own. Bootstrap
95 percent intervals over ten thousand path resamples are 0.175 to
0.325, 0.15 to 0.287, 0.10 to 0.125, 0.175 to 0.375, and 0.05 to
0.075, with 0.10 to 0.15 for Qwen2.5-0.5B's endogenous field and
0.075 to 0.10 for Qwen2.5-1.5B's, every upper bound below the
corresponding null median except the endogenous GPT-2 and Phi-1.5.
...
connected regardless of how the data clusters.
...
is why \theoremref{thm:approx-odd} carries the approximate version,
whose near-boundary point survives any defect, and every kept path
crosses zero regardless. The width column of
\tableref{tab:experiment} likewise gives the slack diagnostic of
\sectionref{sec:geometry} an empirical proxy, 0.11 to 0.37.
...
crosses zero, at least 98 percent cross exactly once, and the
coupling tightens or holds, gaps 0.15, 0.20, 0.075, 0.175, and
0.05, confidence at the crossing 0.42, 0.50, 0.42, 0.38, and 0.49.
The coupling is not an artifact of linearity.

\paragraph{An adversarial falsification attempt.}
The strongest test of an impossibility result is a maximally
empowered attempt to violate it. We freeze the truth field and
train a confidence head on the forbidden objective, above one half
where the field says true and below elsewhere, guarantee weighted
five to one, on all non-test statements plus fresh interpolants
each epoch, evaluated on 21{,}000 to 30{,}000 held-out path points
per model. \theoremref{thm:approx} predicts a positive slack floor,
...
only buys a thinner tube around a boundary that does not go away.
...
addresses the shared-target concern where the model can verbalize
...
hypothesis can be relaxed at a named price
...
feasible truth slack as a measurable diagnostic. The results are
existence statements, so nothing follows about error rates,
\theoremref{thm:dichotomy} is where falsehood is derived, and the
truth field idealizes probe behavior, \theoremref{thm:approx-odd}
pricing oddness. Next steps are higher Borsuk-Ulam machinery and
nonlinear probe boundaries \citep{engels2025not}, in brains and
machines alike, where geometry can prove theorems and machines can
check them.
