...
We fit a GP (RBF, $\sigma{=}0.2$) to the 82 cells and construct a smooth oblique defense $D(x) = x + \alpha\,\beta(\mu(x))\,v(x)$ displaced $89.5^\circ$ from the GP gradient. Empirical estimates: $\hat\ell = 0.86$, $\hat K = 1.05$, $\hat G = 23.6$, so $\hat\ell(\hat K{+}1) = 1.77 \ll \hat G$ and transversality holds non-trivially. Result: $|\mathcal{S}_{\text{pred}}| = 3$, all actually persistent ($\mathrm{TP}{=}3$, $\mathrm{FP}_{\mathrm{int}}{=}0$; \Cref{fig:theory-vs-reality-saturated}).
...
\Cref{tab:llamaguard-demo,tab:forced-collapse,tab:paraphrase-unsafe}.
...
\paragraph{Discrete validation.}
On the same 82-cell archive, Llama-Guard-3-1B passes 51/82 prompts as \texttt{safe} while collapsing to 7 categories; ShieldGemma-2B passes 56/82 while collapsing to 2 (\Cref{fig:classifier-gate-maps}). Both sit on the injective-incomplete horn of \Cref{thm:disc-dilemma}. For the forced-collapse test (\Cref{tab:forced-collapse} in appendix), a refusal defense collapses 66 unsafe inputs to 1 output, a canonical-category defense to 7, while an injective paraphrase defense produces 66 distinct rewrites—of which 1 remains above $\tau$ (\Cref{tab:paraphrase-unsafe}), the predicted incomplete witness. The well-aligned target (gpt-5-mini, peak AD $= 0.5$) yields $U_\tau = \emptyset$ and the theorems are correctly silent.
