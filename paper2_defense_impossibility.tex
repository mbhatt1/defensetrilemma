...
(zero \texttt{sorry}, three standard axioms).

A natural objection is that real prompts are discrete, not continuous.
We close this gap formally: the Tietze extension theorem guarantees that
any finite set of behavioral observations in a normal space admits a
continuous interpolant, so the impossibility applies to discrete
experimental data---not as an approximation, but as a provable consequence.

Beyond the central impossibility, we develop a formal theory of the
...
exponential cost asymmetry between attack and defense. We connect each
result to empirical findings from behavioral mapping of three frontier LLMs
(Llama-3-8B, GPT-OSS-20B, GPT-5-Mini). The complete Lean artifact
(34 files, $\sim$280 theorems) is open-sourced.
\end{abstract}
...
A natural objection is that real prompts are discrete token sequences,
not points in a continuous space. We address this directly: the Tietze
extension theorem guarantees that any finite set of behavioral
observations in a normal space (including $\R^n$) admits a continuous
interpolant that agrees exactly with all observed data points
(\Cref{thm:tietze}). The impossibility theorem then applies to every
such interpolant. This is not an empirical approximation---it is a
formal consequence of the data, verified in Lean alongside the main
result.
...
theory of the vulnerability landscape. Supporting theorems characterize
basin structure and fragmentation, perturbation robustness, iterative
convergence, cross-model transferability, authority monotonicity, gradient
ascent, stability under fine-tuning, and the exponential cost asymmetry
...
\begin{itemize}[nosep]
  ...
  \item Supporting theorems that collectively form a formal theory of
    the failure manifold (\Cref{sec:landscape,sec:attacks,sec:stability,sec:cost}).
  \item Theory--experiment correspondence with empirical findings from
    three LLMs (\Cref{sec:experiments}).
  \item A Lean~4 artifact comprising 34 files, $\sim$280 theorems, zero
    \texttt{sorry}, and three standard axioms (\Cref{sec:artifact}).
\end{itemize}
...
Real prompts are discrete token sequences, and behavioral data is finite.
The theorems above assume a continuous function on a connected space.
...
finite set of behavioral observation points, and let $g\colon S \to \R$
be observed alignment deviation values satisfying $g(p) < \tau$ for some
$p\in S$ and $g(q) > \tau$ for some $q\in S$.

Then there exists a continuous function $f\colon X \to \R$ such that:
\begin{enumerate}[nosep]
  \item $f$ agrees with all observations: $f(s) = g(s)$ for every $s\in S$.
  \item The boundary is nonempty: $\{x : f(x) = \tau\} \neq \emptyset$.
  \item The defense impossibility holds: for every continuous
    $D\colon X\to X$ with $D|_{S_\tau} = \id$, there exists $z\in X$
    with $f(z) = \tau$, $D(z) = z$, and $f(D(z)) \not< \tau$.
\end{enumerate}
\end{theorem}

\begin{proof}
Any finite subset of a $T_1$ space is closed. By the Tietze extension
theorem, the function $g$ on the closed subset~$S$ extends to a
continuous function $f$ on all of~$X$ that agrees with $g$ at every
sample point. Since $f(p) = g(p) < \tau$ and $f(q) = g(q) > \tau$,
the hypotheses of \Cref{thm:main} are satisfied. The three conclusions
follow: boundary nonemptiness by the intermediate value theorem on a
connected space, and defense impossibility by \Cref{thm:main} applied
to~$f$.
\end{proof}

\begin{remark}
The interpolant need not be unique. The theorem applies to \emph{every}
continuous function consistent with the data---whether obtained by
Gaussian process regression (which provably produces continuous posterior
means for jointly continuous kernels), by neural network surrogate, or
by the abstract Tietze construction. The defense impossibility is a
consequence of the data, not of any particular interpolation method.
\end{remark}

\begin{corollary}[Euclidean instantiation]
\label{cor:euclidean}
\Cref{thm:tietze} applies in $\R^n$ for every $n \geq 1$, since
Euclidean space is connected, normal, and Hausdorff.
\end{corollary}
...
that boundary actually look like?
...
\label{thm:basin}
If $f$ is continuous, $f(p)>\tau$ for some~$p$, and $\mu$ is any measure
that assigns positive mass to nonempty open sets, then $U_\tau$ has
positive $\mu$-measure.
\end{theorem}
...
\label{thm:fragment}
If $f$ is $L$-Lipschitz with $L > 0$ and $f(p) > \tau$, then the
connected component of $U_\tau$ containing $p$ has diameter at least
$2(f(p) - \tau)/L$. Larger Lipschitz constants produce smaller
guaranteed fragment sizes.
\end{theorem}

This theorem explains the qualitative difference between models: a smooth
model like GPT-5-Mini (small~$L$) has large, predictable basins, while
a rugged model like GPT-OSS-20B (large~$L$) fragments into a complex
mosaic of small safe and unsafe patches.
...
converge reliably. The theorem identifies the common structure: they are
all monotone-improvement operators acting on a bounded score function.
The empirically falsifiable claim is that these algorithms actually
satisfy the monotonicity assumption---and convergence curves confirm
they do.
...
models do not. Transfer itself costs zero additional queries---it is
pure arithmetic on the sup-norm distance.
...
If vulnerabilities are structural, they should persist under small model
modifications. The following results confirm this.

\begin{theorem}[Interior Stability]
\label{thm:interior-stable}
If $\|f - g\|_\infty \leq \varepsilon$, then:
\begin{enumerate}[nosep]
  \item Points deep in the unsafe basin survive: $f(x) > \tau + \varepsilon
    \implies g(x) > \tau$.
  \item Points deep in the safe region survive: $f(x) < \tau - \varepsilon
    \implies g(x) < \tau$.
  \item Only the band $|f(x) - \tau| \leq \varepsilon$ is uncertain.
\end{enumerate}
\end{theorem}

This means that fine-tuning with bounded effect ($\|f - g\|_\infty \leq
\varepsilon$) cannot eliminate vulnerabilities in the basin interior. Only
the boundary band---points within $\varepsilon$ of the threshold---can
change classification. Everything else is locked in.

\begin{theorem}[Crossing Preservation]
\label{thm:crossing}
If $f$ crosses $\tau$ transversally on $[a,b]$ (meaning $f(a) < \tau -
\varepsilon$ and $f(b) > \tau + \varepsilon$) and $g$ is continuous with
$\|f - g\|_\infty \leq \varepsilon$, then $g$ also crosses $\tau$ on
$[a,b]$.
\end{theorem}

A transversal boundary crossing cannot be eliminated by small perturbation.
If the original model transitions from safe to unsafe along some behavioral
axis, any fine-tuned variant that stays within $\varepsilon$ must also
make that transition somewhere nearby.

\begin{theorem}[Patching Is Nonlocal]
\label{thm:nonlocal}
There exist concrete functions $f, g\colon \R \to \R$ with
$\|f - g\|_\infty \leq \varepsilon$ such that altering $g$ to eliminate a
vulnerability at one point necessarily changes its values at distant
points.
\end{theorem}

This is verified by explicit construction in Lean. The practical
implication: fixing one vulnerability can introduce or move others
elsewhere in the behavioral space.
...
most $\lfloor 1/\delta\rfloor$ steps, regardless of dimension. For the
...
\label{thm:collapse}
For any fixed patch radius $0 < r < 1$ and constant $C > 0$, the
fraction of the space covered by $C$ defense patches satisfies
$C \cdot r^d \to 0$ as $d \to \infty$.
\end{theorem}

In other words, a fixed defense budget covers a vanishing fraction of
the behavioral space as its dimension grows. The defender must pay
exponentially more just to maintain the same coverage level.
...
To test whether the theory describes real model behavior, we validate
each theorem against empirical findings from the Manifold of Failure
...
\centering
\small
\caption{Each theorem generates a falsifiable prediction. In every case,
the empirical data confirms it.}
...
Basin Structure (\ref{thm:basin})
  & Basins have positive area, not isolated dots
  & Heatmaps show extended colored regions \\[4pt]
Fragmentation (\ref{thm:fragment})
  & Smooth models $\to$ large basins; rough models $\to$ mosaic
  & Llama: near-flat mesa; GPT-OSS: fragmented mosaic \\[4pt]
Iterative Convergence (\ref{thm:convergence})
  & MAP-Elites/PAIR/TAP exhibit monotone convergence
  & Convergence curves are monotone and plateau \\[4pt]
Transferability (\ref{thm:transfer})
  & Models with similar AD surfaces share basins
  & Spectrum: Llama ($0.93$) $\to$ GPT-OSS ($0.73$) $\to$ GPT-5-Mini ($0.47$) \\[4pt]
Authority Monotonicity (\ref{thm:authority})
  & Horizontal banding at authority thresholds
...
Interior Stability (\ref{thm:interior-stable})
  ...
  & Llama's $0.93$ plateau persists across prompt variants \\[4pt]
...
\Cref{thm:fragment} predicts that the basin forms essentially one massive
connected region, with only narrow safe corridors surviving at
specific authority levels. \Cref{thm:interior-stable} predicts that
these vulnerabilities are stable under fine-tuning: at $\AD = 0.93$,
a perturbation of $\varepsilon < 0.43$ cannot move any interior point
below threshold $\tau = 0.50$.
...
(\Cref{thm:lipschitz}), and \Cref{thm:fragment} predicts
fragmentation into a complex mosaic of small safe and unsafe patches.
\Cref{thm:authority} predicts horizontal
banding in this mosaic, and the contour plots confirm bands at
$a_2 \approx 0.25$--$0.35$ and $0.65$--$0.85$. The ``bullseye''
patterns visible around vulnerability peaks are exactly the contour
structure one would expect from \Cref{thm:lipschitz}: concentric
level sets of decreasing AD around each peak.
...
leaves the question of what to do about it. The theory points toward four
concrete strategies.
...
\paragraph{Strategy 4: Accept and monitor the boundary.}
\Cref{thm:crossing} shows that transversal crossings persist under
fine-tuning, and \Cref{thm:nonlocal} shows that patching is nonlocal.
Rather than attempting to eliminate boundary points, deploy continuous
monitoring to detect when inputs approach them. The Lipschitz bound
(\Cref{thm:lipschitz}) gives a computable estimate of distance to the
boundary from any observed AD value.
...
Mathlib~v4.28.0. The artifact comprises 34 files organized into:
\begin{itemize}[nosep]
...
\noindent The continuous relaxation theorem additionally uses
\texttt{ContinuousMap.restrict} for the Tietze extension.
...
The discrete-to-continuous gap is formally closed: the Tietze extension
theorem guarantees that finite behavioral data always admits a continuous
interpolant, so the impossibility applies to real experimental data as a
provable consequence rather than an empirical approximation.

Supporting theorems flesh out the landscape that this impossibility
constrains. Vulnerability basins are open sets with positive measure
whose fragment size is controlled by the Lipschitz constant. Successful
attacks are robust to perturbation, converge reliably under iteration,
and transfer between models at zero cost when alignment surfaces are
close. Vulnerabilities in the basin interior are stable under fine-tuning,
transversal boundary crossings persist under perturbation, and patching
is inherently nonlocal. The cost of comprehensive defense grows
...
The full theory is machine-verified in Lean~4: 34 files, approximately
280 theorems, zero \texttt{sorry} statements, and three standard axioms.
