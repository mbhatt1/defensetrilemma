...
- **Defense:** `bounded_step` (params: `{'max_step': 2}`)
...
| `K` | `3.6056` | Lipschitz constant of D |
...
- **Bound:** `|f(D(x)) − τ| ≤ L·K·dist(x, z*) + |f(z*) − τ|` with `LK = 85.1811`, slack `= 0.3750`
...
|---|---|---|---|
| Maximum across all filled cells | `104.6724` | `0.5000` | within |
...
- **Transversality `G > ℓ(K+1)`:** `23.6250 > 108.8061` → **✗ DOES NOT HOLD**
...
| `actual persistent` | `{x : f(D(x)) > τ}` | **47** |
...
| ⚠ False negative | **47** | actually persistent BUT NOT in the predicted steep set — NOT a theorem violation; happens when the defense is too weak in *reach*, not in Lipschitz constant |

✅ **Containment confirmed**: every cell in the predicted steep set (1 cells) is in the actual persistent set (47 cells). Theorem 6.2 holds empirically — `steep_set ⊆ persistent_set`.
