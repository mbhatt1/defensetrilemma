...
- **Defense:** `bounded_step` (params: `{'max_step': 3}`)
...
| `K` | `4.0000` | Lipschitz constant of D |
...
- **Bound:** `|f(D(x)) − τ| ≤ L·K·dist(x, z*) + |f(z*) − τ|` with `LK = 94.5000`, slack `= 0.3750`
...
|---|---|---|---|
| Maximum across all filled cells | `116.0825` | `0.5000` | within |
...
- **Transversality `G > ℓ(K+1)`:** `23.6250 > 118.1250` → **✗ DOES NOT HOLD**
...
| `actual persistent` | `{x : f(D(x)) > τ}` | **29** |
...
| ⚠ False negative | **29** | actually persistent BUT NOT in the predicted steep set — NOT a theorem violation; happens when the defense is too weak in *reach*, not in Lipschitz constant |

✅ **Containment confirmed**: every cell in the predicted steep set (1 cells) is in the actual persistent set (29 cells). Theorem 6.2 holds empirically — `steep_set ⊆ persistent_set`.
