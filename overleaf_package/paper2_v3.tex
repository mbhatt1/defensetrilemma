...
expectation. The theory is \textbf{mechanically verified in Lean~4
with Mathlib} across 46 files containing ${\sim}360$ theorems, with no
admitted proofs and only three standard axioms
...
\paragraph{What is different about our impossibility.}
...
This section removes the most obvious objection to the continuous
results: perhaps the impossibility is an artifact of topology or
continuous interpolation. It is not. On finite sets, the same failure
appears as a combinatorial tradeoff between completeness and
information preservation.

The discrete and continuous views are compatible. If one wishes to
interpolate discrete observations into a continuous prompt space, the
Tietze/McShane--Whitney bridge is provided in the appendix
(\Cref{thm:tietze}). The main paper, however, treats the discrete
theorem directly rather than routing it through continuous relaxation.

\subsection{Discrete Impossibility}
...
structural constraint: an information-preserving defense \emph{cannot}
eliminate unsafe outputs. Any complete defense must destroy
information by collapsing distinct inputs to the same output. We
illustrate that mechanism empirically in \Cref{sec:experiments}.

\paragraph{Falsifier.}
An empirical counterexample would be a complete utility-preserving
classifier $D$ with $|D(X)| = |X|$ on any finite input set: i.e., an
injective defense that nevertheless remaps every unsafe input below
threshold.
...
The continuous and discrete theorems handle single-shot deterministic
wrappers. This section tests two common escape hatches: repeated
interaction and randomization. Neither removes the obstruction.
...
turns --- i.e., an empirically observed sequence in which the
attacker's best-so-far alignment-deviation score goes down --- would
refute the monotone-improvement mechanism underlying
\Cref{thm:multi-turn}.

\paragraph{Empirical illustration.}
\Cref{tab:multi-turn} and \Cref{fig:multi-turn-plot} report a
multi-turn run in which per-turn boundary fixation recurs and the
attacker's running-max AD increases monotonically, as predicted.

\begin{table}[h]
\centering
\small
\caption{Per-turn boundary-fixation statistics for a multi-turn
attack trajectory. Running-max AD is monotone non-decreasing.}
...
\caption{Multi-turn running-max AD trajectory; at each turn a
boundary-fixed point is located, consistent with
\Cref{thm:multi-turn}.}
...
and $f(D(z)) < \tau$ --- i.e., a deterministic-at-$z$ stochastic
wrapper that nevertheless achieves $g(z) < \tau$ --- would refute
\Cref{thm:stochastic}.

\paragraph{Empirical illustration.}
\Cref{tab:stochastic} and \Cref{fig:stochastic-histogram} report
the $f(D(z))$ distribution for a genuinely random wrapper at a
boundary cell: positive variance accompanies positive mass above~$\tau$,
as the dichotomy predicts.
...
variance pushes non-zero mass above~$\tau$.}
...
The empirical section serves one purpose: to show that the paper's
theory is not vacuous. We therefore focus on two tests aligned to the
main theorem families. First, we exercise the continuous theory on
real LLM safety surfaces and check whether the predicted boundary and
persistent-failure structure appears under continuous defenses.
Second, we exercise the discrete theorem on a saturated unsafe-prompt
archive and check whether complete defenses visibly induce collapse of
distinct inputs.

We use the MAP-Elites-based behavioral-space driver
of~\cite{munshi2026manifold}, which maps model behavior over a 2D
prompt space with axes \emph{query indirection} and \emph{authority
framing}. The driver's role is purely methodological; every
alignment-deviation value we report comes from our own live runs,
enumerated and cross-referenced in~\Cref{app:exp-details}.

\subsection{Continuous-Setting Validation}
\label{sec:live-validation}

To \emph{exercise} the persistence theorem on
live alignment-deviation surfaces and to expose every intermediate
quantity, we built a small validator
that takes any 2D heatmap, estimates the empirical quantities
$L, K, \ell, G$, identifies a boundary anchor $z^{*}$, and tests
the observable content of Theorem~\ref{thm:persistent}:
set containment
\[
...
\paragraph{From measure to count.}
The cell-level check is equivalent to the measure-theoretic claim
under the validator's grid approximation: each filled cell represents
a positive-measure neighborhood (a Voronoi cell under the $L_2$
metric), and containment of cell-indices encodes the measure-theoretic
containment $\mathcal{S}_{\text{pred}} \subseteq
\mathcal{S}_{\text{act}}$ up to the discretization slack $|f(z^*) -
\tau|$, which vanishes as the grid refines. Hence
$\mathrm{FP}_{\mathrm{int}} = 0$ on any grid is the discrete analogue
of $\mu(\mathcal{S}_{\text{pred}} \setminus \mathcal{S}_{\text{act}})
= 0$ in the measure-theoretic statement of
\Cref{thm:persistent}, and the vanishing of the slack in
\Cref{tab:resolution} is what justifies treating cell counts as
measure estimates.
...
saturated run is the only configuration in which the theorem can be
non-vacuously exercised. \textbf{Judge dependence.}
...
To enter the anisotropic regime
($\ell < L$, $G > \ell(K{+}1)$) where Theorem~\ref{thm:persistent}
has non-trivial content, we fit a 2D Gaussian Process to the
$82$ filled cells of the saturated \texttt{gpt-3.5-turbo-0125}
grid (RBF kernel, $\sigma = 0.2$) and construct a
\emph{smooth oblique defense}
$D(x) = x + \alpha\,\beta(\mu(x))\,v(x)$, where $v(x)$ is displaced
$89.5^{\circ}$ from the negative gradient of the GP posterior. This
gives empirical $\hat\ell = 0.86$, $\hat K = 1.05$, and
$\hat\ell(\hat K{+}1) = 1.77$, well below $\hat G = 23.6$, so
transversality holds. The predicted steep set contains
$n = 3$ cells, all of which are actually persistent
($\mathrm{TP} = 3$, $\mathrm{FP}_{\mathrm{int}} = 0$;
\Cref{fig:theory-vs-reality-saturated}).
...
Appendix sweeps over three smooth continuous defense families, multiple
kernel choices, and multiple grid resolutions preserve the same main
result: no interior false positive is observed in the continuous regime
when the theorem's hypotheses hold.
...
Bootstrap uncertainty, seed replication, alternative-target
re-evaluation, higher-dimensional distance checks, and judge-robustness
analyses all preserve the key empirical claim: no interior
counterexample is observed in the continuous-defense regime where the
theorem's hypotheses hold. Full tables and estimator details are
reported in the appendix.

\paragraph{Three-target sweep.}
To widen the empirical base we hold prompts and judge fixed and swap
the target model. \Cref{tab:three-target} reports the
continuous-defense validator (\texttt{smooth\_nearest\_safe}) across
the canonical \texttt{gpt-3.5-turbo-0125} archive plus two retargeted
surfaces generated by the concurrent async driver in
\texttt{scripts/retarget\_archive.py} (\texttt{gpt-4o-mini-2024-07-18}
and \texttt{gpt-4o-2024-08-06}). $\mathrm{FP}_{\mathrm{int}} = 0$
holds in every row.
...
models. All rows share the 82 prompts from the saturated archive and
the canonical gpt-4.1 judge; only the target varies.
$\mathrm{FP}_{\mathrm{int}} = 0$ holds across all three.}
\label{tab:three-target}
\input{tables/three_target_sweep.tex}
\end{table}

\paragraph{LlamaGuard demonstration.}
\Cref{tab:llamaguard-demo} replays the forced-collapse demonstration
with LlamaGuard~\cite{inan2023llama} acting as a production
classifier-style wrapper: complete defenses collapse, injective
paraphrases leak, as predicted.

\begin{table}[h]
\centering
\small
\caption{Llama-Guard-3-1B (via local Ollama) applied to the $82$
unsafe-seed prompts of the saturated archive, demonstrating
\Cref{thm:disc-dilemma}'s injective-incomplete horn on a production
safety classifier. 52 of 82 attacks are waved through as \texttt{safe}
(false negatives), while the 30 flagged cases are distributed across
6 of 14 LlamaGuard categories.}
\label{tab:llamaguard-demo}
\input{tables/llamaguard_demo.tex}
\end{table}

\subsection{Discrete Forced-Collapse Demonstration}

We illustrate \Cref{thm:disc-dilemma} on the $66$ unsafe prompts of the
saturated \texttt{gpt-3.5-turbo-0125} archive. Three defenses are
applied, and for each we count how many distinct outputs the $66$
inputs produce (\Cref{tab:forced-collapse}). A refusal defense
$D(x) = \texttt{[REFUSAL]}$ is complete by construction and collapses
all $66$ inputs into one output. A canonical-category defense is also
complete by construction and collapses $66$ inputs to $7$ distinct
labels. A paraphrase defense (\texttt{gpt-4o-mini} at $T=0$) produces
$66$ distinct rewrites and is injective on this sample. By
\Cref{thm:disc-dilemma}, an injective utility-preserving defense must
be incomplete: at least one paraphrase must remain unsafe. The table
makes both horns of the discrete impossibility visible.
...
\paragraph{Completing the injective horn.}
We close the loop by rescoring the $66$ paraphrases through the same
target+judge pipeline and counting how many still produce
$f(D(x))>\tau$ (\Cref{tab:paraphrase-unsafe}). One paraphrase out of
$66$ strict-exceeds $\tau$ under the canonical judge --- the
empirical witness that the injective paraphrase defense is indeed
incomplete, exactly as Theorem~\ref{thm:disc-dilemma} predicts.

\begin{table}[h]
\centering
\small
\caption{Paraphrase defense is injective on the $66$ unsafe prompts
(Table~\ref{tab:forced-collapse}), so Theorem~\ref{thm:disc-dilemma}
requires it to be incomplete. Rescoring confirms exactly one strict
false negative out of $66$.}
\label{tab:paraphrase-unsafe}
\input{tables/paraphrase_unsafe.tex}
\end{table}

The well-aligned target additionally exhibits the harmless-boundary
case: at $\tau = 0.5$, gpt-5-mini's peak AD is exactly $0.5$, so
$U_\tau = \emptyset$ and the impossibility theorems are correctly
silent. The validator and full per-cell records are included in the
artifact; appendix tables report the smoke tests, kernel and
resolution sweeps, bootstrap uncertainty, judge dependence, and
out-of-scope discrete stress tests.
...
accept threshold. This is a concrete, numerically-grounded design
constraint on any wrapper deployed against this alignment surface.
