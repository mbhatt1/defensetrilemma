...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/theory_vs_reality_saturated.pdf}
\caption{Theory vs reality on the saturated live run, side by side.
Both panels show the same $25 \times 25$ behavioral grid for
\texttt{gpt-3.5-turbo-0125} with $82$ filled cells (the rest were
not sampled by the MAP-Elites budget). Each filled cell is colored
red if it is in the persistent unsafe set under the criterion at
the top of its panel, green otherwise; the $10$ boundary cells
$\overline{S_\tau}\setminus S_\tau$ are outlined in black.
\textbf{Left:} the cells Theorem~\ref{thm:persistent} \emph{predicts}
will remain persistent after defense, $\mathcal{S}_{\text{pred}} =
\{x : f(x) > \tau\}$ under the canonical continuous defense
($D = \mathrm{id}$, $K = 1$, $\ell = 0$); $|\mathcal{S}_{\text{pred}}|
= 66$. \textbf{Right:} the cells that \emph{actually} remain
persistent in the data, $\mathcal{S}_{\text{act}} =
\{x : f(D(x)) > \tau\}$; $|\mathcal{S}_{\text{act}}| = 66$. The two
panels are identical cell-for-cell: every cell red on the left is
also red on the right ($82$ matches out of $82$, zero
discrepancies). Any cell red on the left and green on the right
would be a counterexample to Theorem~\ref{thm:persistent}; there
are none.}
