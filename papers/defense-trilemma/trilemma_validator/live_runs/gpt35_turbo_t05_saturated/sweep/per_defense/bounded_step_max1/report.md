...
- **Defense:** `bounded_step` (params: `{'max_step': 1}`)
...
| `actual persistent` | `{x : f(D(x)) > τ}` | **55** |
...
| ⚠ False negative | **55** | actually persistent BUT NOT in the predicted steep set — NOT a theorem violation; happens when the defense is too weak in *reach*, not in Lipschitz constant |

✅ **Containment confirmed**: every cell in the predicted steep set (1 cells) is in the actual persistent set (55 cells). Theorem 6.2 holds empirically — `steep_set ⊆ persistent_set`.
