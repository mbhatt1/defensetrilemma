...
\author{
    {\rm Manish Bhatt\thanks{Corresponding author: manish.bhatt13212@gmail.com}}\textsuperscript{\space}\footnotemark[2]\\
    OWASP
    \and
...
(44 files, ${\sim}340$ theorems, no admitted proofs, three standard
...
The key insight is a bound relative to the \emph{input} value, not just
the threshold~$\tau$. While the $\varepsilon$-robust bound measures
distance from~$\tau$, the input-relative bound measures how much the
defense can change a point's \emph{own} score:
...
bound of \Cref{thm:eps-robust} whenever $f(x) > \tau$.
\end{lemma}
...
\leq (K{+}1)\dist(x, z)$.
Lipschitz: $|f(D(x)) - f(x)| \leq L(K{+}1)\dist(x, z)$.
\end{proof}

In plain terms: the defense can reduce any point's score by at most
$L(K{+}1)$ times its distance from~$z$. If the score rises faster
than that, the defense loses.

\begin{definition}[Steep region]
\label{def:steep}
Given a fixed boundary point $z$, the \emph{steep region} is
$\mathcal{S} = \{x \in X : f(x) > \tau + L(K{+}1)\,\dist(x, z)\}$---the
...
directional slope exceeding~$L(K{+}1)$ at the boundary:
...
$L(K{+}1)\delta$ (dashed red), the region above~$\tau$ persists
...
\label{thm:cone}
In $\R$, if $f$ has directional growth rate $c > L(K{+}1)$ at
...
\label{thm:dilemma}
Let $G = |\nabla f(z)|$ at boundary point~$z$, and define
$K^* = G/L - 1$. Then:
\begin{enumerate}[nosep]
  \item If $K < K^*$: the persistent unsafe region exists
    ($G > L(K{+}1)$, \Cref{thm:persistent} applies).
  \item If $K \geq K^*$: the defense distorts space at least as much
    as the alignment gradient.
\end{enumerate}
No value of $K$ avoids both failure modes simultaneously. The two
regimes meet at $K^*$, the optimal defense aggressiveness.
\end{theorem}
...
the safe endpoint of the boundary crossing from \Cref{thm:disc-ivt}
is fixed: $D(i) = i$.
\end{theorem}

In the discrete setting, the bottleneck is not continuity but
\emph{capacity}: there are simply too many attack configurations for a
constrained defense to handle.
...
fewer than $m$ distinct behaviors, at least one configuration is
mishandled (pigeonhole).
\end{theorem}

\begin{theorem}[Exponential Attack Surface]
\label{thm:budget}
The attack surface grows as $2^d \geq d + 1$. For any fixed budget~$B$,
there exists $d_0$ such that $2^d > B$ for all $d \geq d_0$.
\end{theorem}
...
Even when the defense has equal computational resources to the
attacker, it starts at a structural disadvantage:

\begin{theorem}[Capacity Parity Disadvantage]
\label{thm:capacity}
Let the defense and attacker both operate on $N$ inputs, of which
$|S_\tau|$ are safe. The defense must implement identity on all safe
inputs, consuming $|S_\tau|$ of its capacity. Under capacity parity,
the defense's effective free capacity is $N - |S_\tau| < N$, giving
the attacker an advantage of $|S_\tau|$ units.
\end{theorem}

Utility preservation is a tax on the defense that the attacker does
not pay.
...
crossings far exceeds $L(K{+}1)$ for any reasonable~$K$, placing
...
transversality threshold $L(K{+}1)$ for persistence, and shrinks the
...
\paragraph{5. Break capacity parity with architectural separation.}
Under equal resources, the defense loses capacity to utility
preservation (\Cref{thm:capacity}). Separate the defense from the
utility pipeline---e.g., a lightweight classifier that flags, paired
with a separate response generator---to sidestep the capacity tax.
...
$x \mapsto f(x) - L(K{+}1)\dist(x,z)$ at level~$\tau$, hence open.

\textbf{(2)} Open and nonempty implies positive measure.

\textbf{(3)} For $x \in \mathcal{S}$:
$f(x) > \tau + L(K{+}1)\dist(x,z)$. By \Cref{lem:input-bound}:
$f(D(x)) \geq f(x) - L(K{+}1)\dist(x,z) > \tau$.
\end{proof}
...
and crosses $L(K{+}1)$, transversality is reachable by IVT.

\paragraph{Stochastic regularity.}
$g(x) = \mathbb{E}[f(D(x))]$ satisfies $g - f = 0$ on $\cl{S_\tau}$.

\paragraph{Discrete trilemma.}
On finite ordered sets: utility preservation fixes the safe crossing
endpoint; bounded capacity is overwhelmed by exponential attack surface.
...
available at
\url{https://github.com/mbhatt1/stuff/tree/main/ManifoldProofs}.
The artifact comprises 44 files:
\begin{itemize}[nosep]
