...
\texttt{gpt-3.5-turbo-0125} archive: a refusal defense
$D(x)=\texttt{[REFUSAL]}$ is trivially complete and collapses all
$66$ inputs to $1$ output. A canonical-category defense collapses
them to $7$. A paraphrase defense (\texttt{gpt-4o-mini}, $T=0$) is
injective on this sample ($66$ distinct outputs), so
by~\Cref{thm:disc-dilemma} it must be \emph{incomplete}.

\begin{table}[h]
\centering
\small
\caption{Forced-collapse demo of~\Cref{thm:disc-dilemma}: the two
complete defenses are explicitly non-injective; the injective
paraphrase defense must therefore be incomplete.}
...
$L, K, \ell, G$, a boundary anchor $z^{*}$, and a predicted steep
set. Formal definitions follow in~\Cref{sec:estimators}; the
observable content of Theorem~\ref{thm:persistent} is the set
containment
\[
...
be non-vacuously exercised; it is deferred to the
continuous-defense sweep in~\Cref{sec:gp-instance}. The two small
$25\times 25$ sparse runs are retained as pipeline smoke tests
in~\Cref{sec:identity-sanity}. \textbf{Judge dependence.} All
$f$ values are judge-dependent: swapping the judge can reshape
the surface and thereby change $L, \ell, G$. A preliminary
two-judge comparison is reported
...
bookkeeping, not the theorem.~\Cref{tab:smoketests} reports the
...
\centering
\small
\caption{\textbf{Smoke tests (not a test of the theorem).} Sparse
$25\times 25$ live runs under the canonical identity defense.
Under $D = \mathrm{id}$, $\mathcal{S}_{\text{pred}} \equiv
\mathcal{S}_{\text{act}} \equiv \{x : f(x) > \tau\}$ by
construction, so cell-for-cell agreement is mathematically forced
and only witnesses that the validator's bookkeeping is correct on
low-coverage grids.}
...
Let $G_{f}$ denote the set of filled cells and $h = 1/N$ the
cell width of an $N \times N$ grid.
\textbf{Distance metric.} All distances are the $L_{2}$ metric on
normalized grid coordinates rescaled to $[0,1]^{2}$:
$\dist = h\sqrt{(i_{1} - i_{2})^{2} + (j_{1} - j_{2})^{2}}$.
\textbf{Handling of unfilled cells.} All pairs involving any cell
without a judge-returned score are excluded; no imputation is
performed.
\textbf{Anchor selection.}
$z^{*} = \arg\min_{x \in G_{f}} \lvert f(x) - \tau \rvert$,
restricted to boundary cells $\cl{S_{\tau}} \setminus S_{\tau}$;
ties broken by lexicographic order of grid coordinates $(i, j)$.

\paragraph{Surface Lipschitz~$\hat L$.}
$\hat L = \max_{x, y \in G_{f},\ x \neq y}
\lvert f(x) - f(y) \rvert / \dist(x, y)$; unordered pairs, and
the ordered-pair and unordered-pair maxima coincide.

\paragraph{Defense Lipschitz~$\hat K$.}
$\hat K = \max_{u, v \in G_{f},\ u \neq v}
\dist(D(u), D(v)) / \dist(u, v)$; unordered pairs, symmetric.

\paragraph{Defense-path Lipschitz~$\hat\ell$.}
$\hat\ell = \max_{x \in G_{f},\ D(x) \neq x,\ D(x) \in G_{f}}
\lvert f(D(x)) - f(x) \rvert / \dist(D(x), x)$;
sup over empty set is $0$, so $D = \mathrm{id}$ yields
$\hat\ell = 0$.

\paragraph{Boundary directional rate~$\hat G$.}
$\hat G = \max_{x, y \in G_{f},\ f(x) < \tau < f(y)}
(f(y) - f(x)) / \dist(x, y)$; oriented safe-to-unsafe pairs,
returns $0$ if either filled safe or filled unsafe set is empty.

These match \texttt{lipschitz.py} / \texttt{theorems.py}
exactly; bootstrap CIs on $\hat L, \hat K, \hat\ell, \hat G$ are
reported in~\Cref{tab:ci}.

\paragraph{Higher-dim robustness.}
Distances above use the $2$D MAP-Elites projection; a natural worry
is that $\hat L$ and $\hat G$ are projection artifacts. Recomputing
on the same $82$ cells with pairwise Euclidean distance in the
$768$-d \texttt{all-mpnet-base-v2} embedding (both normalized so
max pairwise distance $=1$) yields $\hat L = \hat G = 17.93$ vs
the $2$D $\hat L = \hat G = 22.68$ — within $21\%$
(\Cref{tab:higher-dim-lipschitz}).

\begin{table}[h]
\centering
\small
\caption{Lipschitz-constant estimates agree within $21\%$ between
the $2$D MAP-Elites grid and the $768$-d MPNet embedding, so the
paper's estimator is not a projection artifact.}
...
because $\ell = 0$ collapses $\mathcal{S}_{\text{pred}}$ onto
$\mathcal{S}_{\text{act}}$ tautologically. To enter the
anisotropic regime ($\ell < L$, $G > \ell(K{+}1)$) where the
theorem has non-trivial content, we fit a 2D Gaussian Process to
the $82$ filled cells of the saturated
\texttt{gpt-3.5-turbo-0125} grid (RBF kernel, $\sigma = 0.2$) and
construct a \emph{smooth oblique defense}
...
safety-seeking component). This gives empirical
$\hat\ell = 0.86$, $\hat K = 1.05$, $\hat\ell(\hat K{+}1) = 1.77$
--- well below $\hat G = 23.6$, so transversality holds. The
predicted steep set contains $n = 3$ cells, all of which are also
actually persistent ($\text{TP} = 3$, $\text{FP}_{\text{int}}
= 0$; \Cref{fig:theory-vs-reality-saturated}). With $n = 3$,
this is a small-sample instance. We treat the oblique GP-smooth
result as one configuration within a multi-kernel,
multi-resolution sweep (Tables~\ref{tab:gp-sensitivity}
and~\ref{tab:resolution}) rather than as standalone validation.
...
$|\mathcal{S}_{\text{act}}| = 68$. Transversality holds
non-trivially ($\hat G = 23.6 \gg \hat\ell(\hat K{+}1) = 1.77$)
and the defense is genuinely non-identity ($82$ cells displaced).
...
The sweep varies the GP kernel $\in
\{\text{RBF}, \text{Mat\'ern-}3/2, \text{Mat\'ern-}5/2\}$
crossed with bandwidth $\sigma\in \{0.1, 0.2, 0.4\}$ (nine
configurations total; \Cref{tab:gp-sensitivity}); grid resolution
is swept independently in~\Cref{tab:resolution}; and the
continuous-defense family is widened beyond the GP-oblique
construction (Gaussian-smoothed nearest-safe and contractive-flow
are also reported) in~\Cref{tab:continuous-sweep}, the main
continuous-defense table.
...
All rows satisfy the transversality hypothesis
$\hat G > \hat\ell(\hat K{+}1)$.}
...
the saturated grid. Kernel $\in
\{\text{RBF}, \text{Mat\'ern-}3/2, \text{Mat\'ern-}5/2\}$ crossed
with bandwidth $\sigma\in\{0.1, 0.2, 0.4\}$; nine configurations.}
...
\centering
\small
\caption{Resolution sweep for the oblique-smooth defense at
progressively finer MAP-Elites grid sizes.}
...
exercised. Alongside the persistence check,
Theorem~\ref{thm:eps-robust}'s $\varepsilon$-robust bound holds
for all $82$ filled cells under each continuous defense tested,
and Theorem~\ref{thm:main}'s boundary fixation is non-vacuously
activated: $10$ boundary cells exist in
$\cl{S_\tau} \setminus S_\tau$ and the closest one to $\tau$ sits
at $\lvert f(z^{*}) - \tau \rvert = 0.375$, the discretization
gap that vanishes under grid refinement.
...
the per-pair finite-difference populations on the saturated
live run.}
\label{tab:ci}
\input{tables/ci.tex}
\end{table}
...
We rerun the full pipeline under an independent RNG state, yielding a
second archive ($76$ filled cells, $21$ safe, $55$ unsafe vs canonical
$82$/$16$/$66$). Surface quantities shift within $\sim6\%$
($\hat L$: $23.62 \to 22.25$; $\hat G$: $23.62 \to 22.25$) and
$\mathrm{FP}_{\mathrm{int}} = 0$ holds in both runs
(\Cref{tab:seed-replication}).

\begin{table}[h]
\centering
\small
\caption{Seed replication: a second independent pipeline run confirms
surface quantities within $\sim 6\%$ and $\mathrm{FP}_{\mathrm{int}}=0$.}
\label{tab:seed-replication}
\input{tables/seed_replication.tex}
\end{table}

\paragraph{Independent target.}
Holding prompts fixed and swapping target from \texttt{gpt-3.5-turbo-0125}
to \texttt{gpt-4o-mini-2024-07-18} (same judge, re-run concurrently in
$\sim 25$s via async client, not MAP-Elites), the better-aligned
target yields $13$ safe, $24$ unsafe, $45$ at-$\tau$ cells (vs
$16$/$66$/$0$). $\mathrm{FP}_{\mathrm{int}}=0$ holds in both runs
...
target swapped. $\mathrm{FP}_{\mathrm{int}} = 0$ under both targets.}
...
them only as a negative consistency check
($\text{FP}_{\text{int}} = 0$ across all discrete defenses;
\Cref{tab:live-sweep}). The predicted steep set collapses to the
anchor cell because the empirical defense-path Lipschitz constant
$\hat\ell = \hat L = 23.63$ on this dense surface forces
$\hat\ell(K{+}1) > \hat G$ for all $K > 0$. This table should
not be read as theorem validation.
...
thus violate a hypothesis of Theorem~\ref{thm:persistent}. The
validator produces $\text{FP}_{\text{int}} = 0$ across all rows
(no out-of-scope counterexamples either). The steep set
collapses to the anchor because $\hat\ell = \hat L = 23.63$
forces $\hat\ell(K{+}1) > \hat G$ for all $K > 0$.}
...
(\texttt{trilemma\_validator/} in the repository). The full
...
\texttt{trilemma\_validator/live\_runs/}.
...
Kendall's $W=0.700$ across all four judges on cell rankings. ``Testable?''
indicates whether this MAP-Elites sample spans both $\mathcal{S}_\tau$
and $\mathcal{U}_\tau$, which is required to locate a boundary cell $z^*$.}
