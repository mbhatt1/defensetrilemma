...
axioms) and illustrated empirically on three LLMs~\cite{munshi2026manifold}.
\end{abstract}
...
We illustrate the mechanism on the $66$ unsafe prompts of the
saturated \texttt{gpt-3.5-turbo-0125} archive. Three defenses
are applied, and for each we count how many distinct outputs the $66$
inputs produce (\Cref{tab:forced-collapse}). A refusal defense
$D(x) = \texttt{[REFUSAL]}$ trivially satisfies \emph{complete} (every
output is safe) and trivially collapses all $66$ inputs into one
output. A canonical-category defense (map each prompt to a harm-category
label) is also complete by construction and collapses $66$ inputs to
$7$ distinct labels. A paraphrase defense (\texttt{gpt-4o-mini} at
$T=0$) produces $66$ distinct rewrites --- it is \emph{injective} on
this sample. By~\Cref{thm:disc-dilemma}, an injective utility-preserving
defense must be \emph{incomplete}: at least one paraphrase must remain
unsafe. The empirical collapse table makes both horns of the discrete
dilemma visible.
...
on the $66$ unsafe prompts of the saturated
\texttt{gpt-3.5-turbo-0125} archive.
The ``refusal'' and ``canonical-category'' defenses are complete by
construction and therefore must be non-injective; they collapse $66$
inputs to $1$ and $7$ distinct outputs respectively.
The paraphrase defense is injective on this sample (all $66$ outputs
distinct) and therefore, by~\Cref{thm:disc-dilemma}, must fail to be
complete on at least one prompt.}
...
\subsection{Illustrative Empirical Exercise of Theorem~\ref{thm:persistent}}
\label{sec:live-validation}

The numbers above are read off the published Manifold of Failure
heatmaps. To \emph{exercise} the persistence theorem on
\emph{independently re-collected} alignment-deviation surfaces and
to expose every intermediate quantity, we built a small validator
that takes any 2D heatmap and computes four empirical quantities
$L, K, \ell, G$, a boundary anchor $z^{*}$, and a predicted
steep set. Formal definitions follow in~\Cref{sec:estimators};
the observable content of Theorem~\ref{thm:persistent} is the
set containment
\[
\mathcal{S}_{\text{pred}} \;=\;
\bigl\{x : f(x) > \tau + \ell(K{+}1)\,\dist(x, z^{*})\bigr\}
\;\subseteq\;
\mathcal{S}_{\text{act}} \;=\; \bigl\{x : f(D(x)) > \tau\bigr\}
\]
under any continuous, $K$-Lipschitz, utility-preserving defense
$D$. We test this as a per-cell confusion matrix:
...
alignment-deviation grid for each target. For
...
saturated run is the only configuration in which the theorem can
be non-vacuously exercised; we therefore defer it to the
continuous-defense sweep in~\Cref{sec:gp-instance}. The two small
$25\times 25$ sparse runs are retained only as pipeline smoke
tests in~\Cref{sec:identity-sanity}. \textbf{Judge dependence.}
All $f$ values are judge-dependent: swapping the judge can
reshape the surface and thereby change $L, \ell, G$. A
preliminary two-judge comparison is reported
in~\Cref{app:judge-robustness}; a comprehensive judge-robustness
study is future work.

\subsubsection{Sanity check: identity defense}
\label{sec:identity-sanity}

When $D = \mathrm{id}$, we have $\ell = 0$ and $K = 1$, and the
predicted steep set
$\mathcal{S}_{\text{pred}} = \{x : f(x) > \tau + 0 \cdot \dist(x, z^{*})\}
= \{x : f(x) > \tau\}$
collapses to $\mathcal{S}_{\text{act}} = \{x : f(\mathrm{id}(x)) > \tau\}
= \{x : f(x) > \tau\}$ identically. With $D = \mathrm{id}$,
$\mathcal{S}_{\text{pred}}$ and $\mathcal{S}_{\text{act}}$
coincide by construction; these runs only confirm the validator's
bookkeeping, not the theorem. We therefore treat the identity
rows below as a pipeline sanity check (is the heatmap loaded
without off-by-one errors? does the confusion matrix tally
correctly?), not as a test of
Theorem~\ref{thm:persistent}.~\Cref{tab:smoketests} reports the
two small $25\times 25$ sparse-grid runs used during validator
development.

\begin{table}[h]
\centering
\small
\caption{\textbf{Smoke tests (not a test of the theorem).} The
two sparse $25\times 25$ live runs under the canonical identity
defense. Under $D = \mathrm{id}$,
$\mathcal{S}_{\text{pred}} \equiv \mathcal{S}_{\text{act}} \equiv
\{x : f(x) > \tau\}$ by construction, so cell-for-cell agreement
is mathematically forced and only witnesses that the validator's
bookkeeping is correct on low-coverage grids. The saturated run
is retained for the continuous-defense experiment in
\Cref{sec:gp-instance}.}
...
Each empirical quantity below is the exact finite-difference
estimator implemented in the validator's \texttt{lipschitz.py}.
Let $G_{f}$ denote the set of filled cells and
$h = 1/N$ the cell width of an $N \times N$ grid.
\textbf{Distance metric.} All distances are the $L_{2}$ metric on
normalized grid coordinates rescaled to $[0,1]^{2}$: for cells
$(i_{1}, j_{1})$ and $(i_{2}, j_{2})$,
$\dist = h \sqrt{(i_{1} - i_{2})^{2} + (j_{1} - j_{2})^{2}}$.
\textbf{Handling of unfilled cells.} All pairs involving any cell
without a judge-returned score are excluded from every
estimator; no imputation is performed.
\textbf{Anchor selection.}
$z^{*} = \arg\min_{x \in G_{f}} \lvert f(x) - \tau \rvert$,
restricted to boundary cells $\cl{S_{\tau}} \setminus S_{\tau}$;
ties are broken by lexicographic order of grid coordinates
$(i, j)$.

\paragraph{Surface Lipschitz constant~$\hat L$.}
\(
  \hat L = \max_{x, y \in G_{f},\ x \neq y}
  \lvert f(x) - f(y) \rvert / \dist(x, y).
\)
All $\binom{|G_{f}|}{2}$ unordered pairs of filled cells; the
absolute value makes the estimator symmetric, so ordered-pair and
unordered-pair maxima coincide.

\paragraph{Defense Lipschitz constant~$\hat K$.}
\(
  \hat K = \max_{u, v \in G_{f},\ u \neq v}
  \dist\bigl(D(u), D(v)\bigr) / \dist(u, v).
\)
All unordered pairs of filled input cells (the defense target
$D(x)$ is defined on every cell; only the input side is required
to be filled). Symmetric in $(u, v)$.

\paragraph{Defense-path Lipschitz constant~$\hat\ell$.}
\(
  \hat\ell = \max_{x \in G_{f},\ D(x) \neq x,\ D(x) \in G_{f}}
  \lvert f(D(x)) - f(x) \rvert / \dist(D(x), x).
\)
Per-cell (not per-pair): only cells whose defense target is also
filled contribute. Sup over the empty set is $0$ by convention,
so $D = \mathrm{id}$ yields $\hat\ell = 0$.

\paragraph{Boundary directional rate~$\hat G$.}
\(
  \hat G = \max_{x, y \in G_{f},\ f(x) < \tau < f(y)}
  (f(y) - f(x)) / \dist(x, y).
\)
Oriented safe-to-unsafe pairs; the rise is non-negative so the
ordered-pair and unordered-pair maxima coincide. Returns $0$ if
either the filled safe set or the filled unsafe set is empty.

\paragraph{Boundary anchor~$z^{*}$ and residual.}
As above; the residual $\lvert f(z^{*}) - \tau \rvert$ is the
discretization gap that vanishes as $h \to 0$ under grid
refinement.

These are exactly the estimators in \texttt{lipschitz.py} and
\texttt{theorems.py}; uncertainty on $\hat L, \hat K, \hat\ell,
\hat G$ is reported by bootstrap in~\Cref{tab:ci}.

\paragraph{Higher-dimensional robustness.}
The estimators above use a $2$D MAP-Elites behavioral projection
for distances, so a natural worry is that $\hat L$ and $\hat G$
are artifacts of the $2$D grid rather than properties of the
underlying semantic space. We address this by recomputing $\hat L$
and $\hat G$ on the same $82$ cells with pairwise distances in the
$768$-dimensional
\texttt{sentence-transformers/all-mpnet-base-v2} embedding
(the same encoder rethinking-evals uses to compute behavioral
descriptors). After normalizing both distance matrices so that the
maximum pairwise distance is $1$, the $768$-d estimates
($\hat L = \hat G = 17.93$) are within $21\%$ of the $2$D estimates
($\hat L = \hat G = 22.68$; ratio $0.79$,
see~\Cref{tab:higher-dim-lipschitz}). The Lipschitz constants are
stable across this dimensional change, so the $2$D projection is not
materially inflating or deflating them.

\begin{table}[h]
\centering
\small
\caption{Empirical $\hat L$ and $\hat G$ computed with two distance
metrics on the same $82$-cell saturated archive. The $2$D metric is
the paper's main estimator (normalized MAP-Elites grid coordinates);
the $768$-d metric uses
\texttt{sentence-transformers/all-mpnet-base-v2} embeddings of the
cell prompts with pairwise-max-normalized Euclidean distance. The two
estimates agree to within $21\%$, so the $2$D projection is not
materially distorting the Lipschitz constants.}
...
The identity defense cannot test Theorem~\ref{thm:persistent}
because $\ell = 0$ collapses
$\mathcal{S}_{\text{pred}}$ onto $\mathcal{S}_{\text{act}}$
tautologically. To enter the anisotropic regime
($\ell < L$, $G > \ell(K{+}1)$) where the theorem has non-trivial
content, we fit a 2D Gaussian Process to the $82$ filled cells of
the saturated \texttt{gpt-3.5-turbo-0125} grid (RBF kernel,
...
posterior (nearly tangent to level curves, with a small
safety-seeking component). This gives empirical $\hat\ell = 0.86$,
$\hat K = 1.05$, $\hat\ell(\hat K{+}1) = 1.77$---well below
$\hat G = 23.6$, so transversality holds. The predicted steep set
contains $n = 3$ cells, all of which are also actually persistent
($\text{TP} = 3$, $\text{FP}_{\text{int}} = 0$;
\Cref{fig:theory-vs-reality-saturated}). With $n = 3$, this is a
small-sample instance. We treat the oblique GP-smooth result as
one configuration within a multi-kernel, multi-resolution sweep
(Tables~\ref{tab:gp-sensitivity} and~\ref{tab:resolution}) rather
than as standalone validation.

\begin{figure}[h]
\centering
% TODO: regenerated by Agent 4 with fixed axis-label clearance
...
The sweep varies the GP kernel $\in \{\text{RBF},
\text{Mat\'ern-}3/2, \text{Mat\'ern-}5/2\}$ crossed with bandwidth
$\sigma \in \{0.1, 0.2, 0.4\}$ (\emph{nine configurations}
total,~\Cref{tab:gp-sensitivity}); grid
resolution is swept independently in~\Cref{tab:resolution}; and
the continuous-defense family is widened beyond the GP-oblique
construction (Gaussian-smoothed nearest-safe and contractive-flow
are also reported) in~\Cref{tab:continuous-sweep}. The main
continuous-defense table is~\Cref{tab:continuous-sweep}.
...
Each row reports empirical $\hat\ell$, $\hat K$, $|\mathcal{S}_{\text{pred}}|$,
$|\mathcal{S}_{\text{act}}|$, TP and FP$_{\text{int}}$. All rows
satisfy the transversality hypothesis $\hat G > \hat\ell(\hat
K{+}1)$.}
...
\centering
\small
\caption{GP-kernel sensitivity for the oblique-smooth defense on
the saturated \texttt{gpt-3.5-turbo-0125} grid. Kernel
$\in\{\text{RBF},\text{Mat\'ern-}3/2,\text{Mat\'ern-}5/2\}$
crossed with bandwidth $\sigma\in\{0.1, 0.2, 0.4\}$, yielding
nine configurations. All configurations are reported with their
empirical $\hat\ell, \hat K, \hat G$ and confusion-matrix counts.}
...
\texttt{gpt-3.5-turbo-0125} at progressively finer MAP-Elites
grid sizes. As $h = 1/N$ decreases, the discretization gap
$\lvert f(z^{*}) - \tau \rvert$ shrinks and $|\mathcal{S}_{\text{pred}}|$
grows; the theorem's containment holds across all
resolutions.}
\label{tab:resolution}
\input{tables/resolution.tex}
\end{table}

We observe zero empirical counterexamples in the
continuous-defense regime where Theorem~\ref{thm:persistent}'s
hypotheses hold, across all rows
of~\Cref{tab:continuous-sweep,tab:gp-sensitivity,tab:resolution}.
The defense is genuinely non-identity ($D \neq \mathrm{id}$, $82$
cells displaced in the GP-oblique base configuration),
transversality $\hat G > \hat\ell(\hat K{+}1)$ is non-trivially
satisfied, and the theorem's containment is empirically
exercised.

Alongside the persistence check, Theorem~\ref{thm:eps-robust}'s
$\varepsilon$-robust bound holds for all $82$ filled cells under
each continuous defense tested, and Theorem~\ref{thm:main}'s
boundary fixation is non-vacuously activated: $10$ boundary cells
exist in $\cl{S_\tau} \setminus S_\tau$, and the closest one to
$\tau$ sits at $\lvert f(z^{*}) - \tau \rvert = 0.375$, the
discretization gap that vanishes under grid refinement.

\paragraph{Bootstrap uncertainty.}
We report bootstrap percentile CIs over $B = 1000$ resamples of
the per-pair finite-difference populations
(Table~\ref{tab:ci}); the medians and
$[5^{\text{th}}, 95^{\text{th}}]$ percentiles for
$\hat L, \hat K, \hat\ell, \hat G$ on the saturated
\texttt{gpt-3.5-turbo-0125} grid are produced by
\texttt{uncertainty.py} in the validator.

\paragraph{Seed replication.}
Bootstrap resamples the same archive; run-to-run variability from
LLM temperature and MAP-Elites RNG is a separate concern. We rerun
the full pipeline (\texttt{gpt-3.5-turbo-0125} target, gpt-4.1 judge,
$100$ seed prompts, $300$ MAP-Elites iterations, $\tau{=}0.5$) under
an independent RNG state, producing a second archive of $76$ filled
cells ($21$ safe, $55$ unsafe) versus the canonical $82$ filled cells
($16$ safe, $66$ unsafe). Surface-level quantities shift within a
$\sim6\%$ band ($\hat L$ drops from $23.62$ to $22.25$, $\hat G$ from
$23.62$ to $22.25$) and both runs confirm
$\mathrm{FP}_{\mathrm{int}} = 0$ under the same continuous defense
(\Cref{tab:seed-replication}). The theorem's empirical claim is
seed-robust.
...
\texttt{gpt-3.5-turbo-0125}. The second row is a second independent
run of the same configuration (no fixed RNG; LLM temperature $0.7$).
Surface quantities agree within $\sim 6\%$ and
$\mathrm{FP}_{\mathrm{int}} = 0$ holds in both runs.}
\label{tab:seed-replication}
\input{tables/seed_replication.tex}
\end{table}

\paragraph{Independent target.}
We further hold the prompts fixed and vary only the target model. The
82 prompts from the saturated archive are re-run (concurrently via
async OpenAI client) against \texttt{gpt-4o-mini-2024-07-18}, then
rescored with the canonical gpt-4.1 judge. The wall clock is $25$
seconds versus the $\sim 2$ hours of a full MAP-Elites re-run; the
resulting surface is genuinely different (peak AD $0.99$ vs $1.00$;
13 safe, 24 unsafe, 45 at-$\tau$ vs $16$/$66$/$0$ under the canonical
target). Theorem 4.1 boundary fixation is activated (7 boundary cells
fixed by the defense). Theorem 5.1's $\varepsilon$-robust bound holds
for all $82$ cells. Theorem 6.3's positive prediction is vacuous under
this defense (transversality fails because $\ell \sim L$ for
\texttt{smooth\_nearest\_safe}), so $|\mathcal{S}_{\mathrm{pred}}|=0$
and FP$_{\mathrm{int}}=0$ trivially. No counterexample
(\Cref{tab:independent-dataset}).

\begin{table}[h]
\centering
\small
\caption{Independent target. Same 82 prompts, judge held at gpt-4.1,
target swapped from gpt-3.5-turbo-0125 to gpt-4o-mini-2024-07-18.
Surface quantities drop roughly $2\times$ as expected for the
better-aligned target; $\mathrm{FP}_{\mathrm{int}} = 0$ holds in both
runs under the same continuous defense.}
...
\centering
\small
\caption{Bootstrap percentile CIs (median,
$[5^{\text{th}}, 95^{\text{th}}]$) over $B = 1000$ resamples of
the per-pair finite-difference populations underlying each
estimator on the saturated live run.}
\label{tab:ci}
\input{tables/ci.tex}
\end{table}

\subsubsection{Stress test: discrete defenses}
\label{sec:discrete-stress}

Bounded-step defenses are not continuous, so
Theorem~\ref{thm:persistent}'s hypotheses do not hold; we include
them only as a negative consistency check. Even though the
theorem is formally silent on these defenses, the validator
reports $\text{FP}_{\text{int}} = 0$ across all discrete defenses
(\Cref{tab:live-sweep}), so nothing in the empirical data
contradicts the theorem out-of-scope either. The predicted steep
set collapses to the anchor cell because the empirical
defense-path Lipschitz constant $\hat\ell = \hat L = 23.63$ on
this dense surface forces $\hat\ell(K{+}1) > \hat G$ for all
$K > 0$. This table should \emph{not} be read as theorem
validation.

\begin{table}[h]
\centering
\small
\caption{\textbf{Pipeline stress test, not a theorem check.}
Defense sweep on the saturated \texttt{gpt-3.5-turbo-0125} grid
($82$ filled cells, $\tau = 0.5$) over \emph{discrete}
\texttt{bounded\_step} defenses, which are non-continuous and
thus violate a hypothesis of Theorem~\ref{thm:persistent}. We
report these only to witness that the validator produces
$\text{FP}_{\text{int}} = 0$ across all rows (no out-of-scope
counterexamples either). The steep set collapses to the anchor
because $\hat\ell = \hat L = 23.63$ forces $\hat\ell(K{+}1) > \hat
G$ for all $K > 0$.}
...
The well-aligned target additionally exemplifies the
harmless-boundary case: at $\tau = 0.5$, gpt-5-mini's peak AD is
exactly $0.5$ and $U_\tau = \emptyset$, so the trilemma's
...
grid above (\texttt{gpt35\_turbo\_t05\_saturated/}), the kernel
sensitivity sweep (\texttt{sensitivity/}), the continuous-defense
sweep (\texttt{continuous\_defenses/}), the bootstrap CI run
(\texttt{bootstrap/}), and the judge-robustness comparison
(\texttt{judge\_robustness/}).
