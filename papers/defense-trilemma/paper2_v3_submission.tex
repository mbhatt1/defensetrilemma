...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Formal verification}: the full theory is verified in
    Lean~4 across 46 files with ${\sim}360$ declarations, no admitted
...
by Lemma~\ref{lem:input-bound} and the definition of~$\mathcal{S}$.
Full proof in \Cref{app:proofs}.
\end{proof}
...
\Cref{thm:disc-dilemma}'s injective-incomplete horn. 51 of 82 attacks pass as \texttt{safe}.}
...
$f(x) > \tau + \ell(K+1)\dist(x,z)$. By Lemma~\ref{lem:input-bound}:
$f(D(x)) \geq f(x) - \ell(K+1)\dist(x,z) > \tau$.
...
$\hat\ell_{\mathrm{diag}}$ is a secondary diagonal estimator used for
relative comparison across sweep rows; the main-text estimator
$\hat\ell$ (\Cref{sec:estimators}) gives the canonical per-cell
value $\hat\ell = 0.86$ for \textsc{ObliqueGP}.}
...
\begin{itemize}[nosep]
  ...
  \item 1 refined persistence with defense-path Lipschitz constant~$\ell$
    ...
    formalization of Lemma~\ref{lem:input-bound},
    Definition~\ref{def:steep}, and Theorem~\ref{thm:persistent}
